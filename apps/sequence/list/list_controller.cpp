...
void ListController::fillTitleCellForRow(VerticalSequenceTitleCell *cell,
  ...
  // Set the color
  KDColor nameColor =
      sequence->isActive() ? sequence->color() : Theme::ThemeGestion::getColor("GrayDark");
...
void ListController::fillExpressionCellForRow(HighlightCell *cell, int row) {
  ...
  Shared::Sequence *sequence = modelStore()->modelForRecord(record);
  // Set the color
  KDColor textColor = sequence->isActive() ? Theme::ThemeGestion::getColor("KDColorBlack") : Theme::ThemeGestion::getColor("GrayDark");
