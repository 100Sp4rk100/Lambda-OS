...
void TestPlotPolicy::drawLabelAndGraduation(const AbstractPlotView *plotView,
  ...
  plotView->drawLayout(ctx, rect, layout, Coordinate2D<float>(x, 0.f),
                       ...
                       AbstractPlotView::RelativePosition::After, Theme::ThemeGestion::getColor("KDColorBlack"));
}
...
void TestPlotPolicy::drawTestCurve(const Shared::AbstractPlotView *plotView,
  ...
  if (op == Poincare::ComparisonNode::OperatorType::Superior) {
    patternBoth = Pattern(true, false, false, true, Theme::ThemeGestion::getColor("PurpleBright"),
                          Theme::ThemeGestion::getColor("YellowDark"));
    singleCurveStart = range->xMin();
    if (z < xAlpha) {
      singleStart = clampedZ;
      patternSingle = Pattern(Theme::ThemeGestion::getColor("YellowDark"));
      bothStart = clampedXAlpha;
    } else {
      singleStart = clampedXAlpha;
      patternSingle = Pattern(true, false, false, true, Theme::ThemeGestion::getColor("PurpleBright"));
      bothStart = clampedZ;
    }
    ...
    patternBoth = Pattern(true, false, true, false, Theme::ThemeGestion::getColor("PurpleBright"),
                          Theme::ThemeGestion::getColor("YellowDark"));
    bothStart = range->xMin();
    if (z < xAlpha) {
      bothEnd = clampedZ;
      patternSingle = Pattern(true, false, true, false, Theme::ThemeGestion::getColor("PurpleBright"));
...

  {
    CurveDrawing plot(Curve2D(evaluate, m_test), nullptr, bothStart, bothEnd,
                      plotView->pixelWidth(), Theme::ThemeGestion::getColor("YellowDark"));
