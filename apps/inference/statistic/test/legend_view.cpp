...
void LegendView::DotView::drawRect(KDContext* ctx, KDRect rect) const {
  ctx->fillAntialiasedCircle(KDPointZero, k_diameter / 2, Theme::ThemeGestion::getColor("KDColorOrange"),
                             Theme::ThemeGestion::getColor("KDColorWhite"));
}

void LegendView::StripedDotView::drawRect(KDContext* ctx, KDRect rect) const {
  ctx->fillCircleWithStripes(KDPointZero, k_diameter / 2, Theme::ThemeGestion::getColor("PurpleBright"),
                             Theme::ThemeGestion::getColor("KDColorWhite"), 3);
}

LegendView::LegendView()
    : m_pValueLabel(I18n::Message::PValue,
                    {.style = {.glyphColor = Theme::ThemeGestion::getColor("GrayVeryDark") ,
